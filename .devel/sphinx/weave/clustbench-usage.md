...
The figure below illustrates the benchmark dataset at hand.

*Implementation: [`genieclust`](https://genieclust.gagolewski.com/)`.plots.plot_scatter`*.
...
Of course, the purpose of this benchmark is also to test new methods.
...
Let us test the k-means algorithm, for example:
...
More functions are listed in the documentation (see the next section).
Also please see {ref}`sec:colouriser` for an interactive standalone application
