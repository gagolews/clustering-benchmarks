...
The **adjusted asymmetric accuracy** {cite}`aaa`, $\mathrm{AAA}$,
is one of the many *external cluster validity measures*
(see the {ref}`Appendix <sec:partition-similarity-scores>` for more details).
...
```{python}
genieclust.compare_partitions.adjusted_aymmetric_accuracy(y_true, y_pred)
```

indicates a decent degree of similarity between the reference
and the discovered partitions.

It is adjusted for chance and cluster size inequality.
In particular, its expected value is equal to 0 if the partitions were
picked at random or all the points are put into a single
large cluster. Also, the relabelling of cluster ID (see above) does not affect
the score.
